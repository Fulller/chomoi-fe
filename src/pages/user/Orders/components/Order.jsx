...
const Order = ({ order }) => {
  ...
  const handleReceived = async (orderId, status) => {
    setIsloading(true);
    const [result, error] = await OrderService.userUpdateOrderStatus(
      orderId,
      status
    );
    if (error) {
  ...
  const handleCancel = async (orderId) => {
    setIsloading(true);
    const [result, error] = await OrderService.userUpdateOrderStatus(orderId, {
      status: "CANCELLED",
    });
          ...
          <div className="flex flex-col p-5">
            <div className="flex flex-row">
              <div className="flex-1 mx-3 my-3">
                <Link
                  to={`/shop/${id}`}
                  className="flex flex-row font-bold w-44 hover:text-primary"
                >
              ...
              <div className="flex flex-row mx-auto my-auto">
                ...
                {
                  <span className={`${statusColor[status]} font-semibold mt-1`}>
            ...
            {orderItems.map((item) => {
              const { id } = item;
              return <OrderItem key={id} orderItem={item} />;
            })}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between bg-white mx-2 mt-1 p-2 rounded w-full">
          {status === "PENDING" ? (
            <div className="flex flex-1 flex-row">
              <button
                className="p-2 ml-4 bg-primary text-white rounded hover:bg-[#f985ae]"
          ...
          {status === "DELIVERED" ? (
            <div className="flex flex-1 flex-row">
              <button
                className="p-2 ml-4 bg-primary text-white rounded hover:bg-[#f985ae]"
                onClick={() => handleReceived(orderId, { status: "RECEIVED" })}
              >
                Đã nhận hàng
              </button>
            </div>
